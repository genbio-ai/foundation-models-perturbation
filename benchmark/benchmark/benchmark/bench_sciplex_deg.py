...
@hydra.main(version_base=None, config_path="config/sciplex", config_name="config_sciplex_deg")
def main(cfg: DictConfig) -> None:
    ...
    if cfg.emb_name == "pca":
        ...
        train_emb = emb[train.obs["drug"].astype(str).tolist()].obsm[emb_name]
        test_emb = emb[test.obs["drug"].astype(str).tolist()].obsm[emb_name]
    ...
    if cfg.estimator_name in {"prior", "most_frequent", "no_change"}:
        ...
        predictions = torch.Tensor(np.stack(estimator.predict_proba(test_emb), axis=1)).transpose(2, 1)
    else:
